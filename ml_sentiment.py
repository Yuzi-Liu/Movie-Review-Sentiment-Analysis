...
def get_tuned_lr(train, dev, features, output_file_path='./lr.png'):
    ...
    cs = np.arange(0.1, 0.3, 0.05)  # You will change this range, or may want to use np.logspace instead of np.arrange
    for c in cs:
...
def get_tuned_rf(train, dev, features, output_file_path='./rf.png'):
    ...
    train_accuracy_list = list()
    dev_accuracy_list = list()
    n_estimators = np.arange(90, 120, 5)  # You will change this range, and try different parameters to tune RF model
